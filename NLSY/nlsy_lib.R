...
# Read the raw data
nlsydf = read_csv('NLSY/NLSY-college-finance_imp.csv') %>%
    select(-"...1")
...
nlsy_recode_race = function(data)
{
  data = data |>
   ...
   data = data |># Creating a joint race_ethnicity column
          mutate(race_eth = fct_recode(
              ifelse(is.na(hisp) & !is.na(race), as.character(race),
              ifelse(!is.na(hisp) & is.na(race), as.character(hisp),
              ifelse(!is.na(hisp) & !is.na(race) & (race=="White"), "White",
              ifelse(!is.na(race) & (race=="Black"), "Black",
              ifelse(!is.na(race) & (race=="Other"), "Other", NA_character_)
              ))))))
...
nlsy_get_col_stat_fall_df = function(data)
{
  data = data |>
    select(
        id,
...
nlsy_get_highest_grade_completed_df = function(data)
{
  data = data |>
    select(
      id =        "PUBID_1997",
...
nlsy_get_famrel_df = function(data)
{
  withpardf = data |>
    select(
      id =        "PUBID_1997",
...
nlsy_get_student_loans_df = function(data)
{
  sloandf = data |>
    select(
      id =        "PUBID_1997",
