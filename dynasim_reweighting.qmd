...
make_multipliers = function(data, varname, targets)
{
...
reweight_by = function(data, varname)
{
    mpdf = data |> make_multipliers(varname, targets)
    df   = data |> apply_multipliers(mpdf, varname)
    return(df)
}

calculate_diff = function(data, varname)
{
    return(
        data |> 
            make_multipliers(varname, targets) |>
...
# Create targets
targets=list(
    inc_pov_r   = make_income_target(places$geoid[1]),
    sexrace     = make_sex_race_target(places$geoid[1])
)
...
df0 = read_csv("X:/programs/Run1006(babybonds)/BabyBonds/BabyBondsData14AUG24.csv") |>
    rename_all(tolower) |>
    filter(age==18) |>
    encode_dyn_race_sex() |>  
...
for(i in 1:maxiter) {
    ...
    {
        df = df |> reweight_by(names(targets)[k])
    }
    ...
    for(k in 1:length(targets)) 
    {
        ...
        diffs[varname] =  df |> calculate_diff(varname)
    }
