...
#        "B01001A",  "Sex by Age (White Alone)",
    ...
    datadf = list_rbind(
        apply(
            places, 
            1, 
            \(x) get_acs_tables(acs1tabs$code, x['state'], x['geoid'])
            )
        )
...
plotdf = df |> group_by(GEOID,sex,agecat,race) |>
...
plotdf = df |> 
  ...
  rename(geoid=GEOID) |>
  left_join(places, by="geoid") |>
  mutate(agecat=factor(agecat, levels=agecatsdf$longage, labels=agecatsdf$shortage))
