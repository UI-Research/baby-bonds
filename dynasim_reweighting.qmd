...
acs1_ten_filename   = paste0(acs_data_dir, "acs2022_ten.rds")
...
get_acs_by_tenure_race = function()
{
    # Tables we need
    acs1tabs = tribble(
        ~code,      ~concept,
#        "B25003A",  "Tenure (White Alone)",
        "B25003B",  "Tenure (Black or African American Alone)",
        "B25003C",  "Tenure (American Indian and Alaska Native Alone)", 
        "B25003D",  "Tenure (Asian Alone)", 
        "B25003E",  "Tenure (Native Hawaiian and Other Pacific Islander Alone)",
        "B25003F",  "Tenure (Some Other Race Alone)",
        "B25003G",  "Tenure (Two or More Races)",
    ...
    #        
    # Fetch the tables
    b25003fname = paste0(here::here(), "/ACS/B25003.rds")
    if( file.exists(b25003fname) ) {
        datadf = readRDS(b25003fname)
    } else {
        datadf = get_acs_tables_for_places(places, acs1tabs$code)
        saveRDS(datadf, b25003fname)
    }
    ...
    labdf = list_rbind(
        ...
        mutate(
            ...
            label=case_when(
                label=="" ~ "All",
                label=="Owner occupied" ~ "Owner",
                label=="Renter occupied" ~ "Renter",
                TRUE ~ label
            ),
            race=gsub("Tenure \\((.+) Householder\\)", "\\1", concept),
            race=gsub(" Alone", "", race),
            race=gsub(" or African American", "", race),
            race=if_else(race=="Hispanic or Latino", "Hispanic", race),
            race=if_else(race=="White, Not Hispanic or Latino", "White", race),
        ...
        #
        # Rename "name", we will use it for a join
        rename(variable=name, tenure=label)
...
if( file.exists(acs1_ten_filename) ) {
    tendf = readRDS(acs1_ten_filename)   
} else {
    tendf = get_acs_by_tenure_race()
    saveRDS(povdf, acs1_ten_filename)
}
...
```{r}
#| label: fig-inc-to-pov-by-age-old
#| fig-cap: "The ratio of household income to poverty level for children ages 1-5"
...
get_dynasim_sample = function()
{
    dynasim_file = "X:/programs/Run1006(babybonds)/BabyBonds/BabyBondsData15AUG24.csv"

    df = read_csv(dynasim_file, show_col_types=FALSE) |>
        rename_all(tolower) |>
        filter(age==0) |>
        encode_dyn_race_sex() |>  
...
get_dynasim_wealth_sample = function()
{
    dynasim_wealth_file = "X:/programs/Run1006(babybonds)/BabyBonds/WealthData16Aug24.csv"
    wealthdf = read_csv(dynasim_wealth_file) |>
        rename_all(tolower)
}
...
# Unfortunately, there is no age information for tenure,
# so we can't use it on the narrow sample with only people
# born in 2024-2028.
make_tenure_race_target = function(geoid)
{
    df = tendf |> 
        filter(
            tenure!="All", 
...
res = map2(res, places$city, \(x,y) mutate(x, city=y)) |>
    list_rbind()

write_csv(res, paste0(here::here(), "/data/DYNASIM/city_weights.csv"))

```


```{r}
#| label: fig-inc-to-pov-lt6
#| fig-cap: "The ratio of household income to poverty level for children younger than 6"

plotdf = bind_rows(
    res |> group_by(city,inc_pov_r) |>
        summarise(population=sum(weight)) |>
        ungroup() |>
        mutate(source="DYNASIM"),
...
plotdf |>
  ggplot() +
  geom_col(aes(x=inc_pov_r, y=population, fill=source), position=position_dodge()) +
  facet_wrap(~city) +
  ylab("Population") +xlab("Income-to-poverty-level ratio") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
...
```{r}
#| label: fig-race-sex-lt5
#| fig-cap: "Shaers of sex and race for children younger than 5"

plotdf = bind_rows(

    res |>
        group_by(city, sexrace) |>
        summarise(population=sum(weight)) |>
        ungroup() |>
        mutate(source="DYNASIM"),
    ...
    asrdf |>
        rename(geoid=GEOID) |>
        mutate(race = if_else(race=="Asian", "Other", race)) |>
        mutate(sexrace=paste0(sex, "-", race)) |>
        filter(agecat=="<5", sex!="All", race!="All") |> 
        group_by(geoid,sexrace) |>
        summarise(population=sum(estimate, na.rm = TRUE)) |>
        ungroup() |>
        left_join(places, by="geoid") |>
        select(city, sexrace, population) |>
        mutate(source="ACS")
)

plotdf |>
  ggplot() +
  geom_col(aes(x=sexrace, y=population, fill=source), position=position_dodge()) +
  facet_wrap(~city) +
  ylab("Population") +xlab("Sex/race") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
