...
asec_df_filtered |> 
    ...
    count(YEAR, age_bucket, SEX, race_eth, wt = ASECWT) |>
    mutate(`Relative Freq` = n/sum(n), .by = c('YEAR', 'age_bucket', 'SEX')) |> 
    ggplot(aes(x=YEAR, y=`Relative Freq`, color=race_eth)) + geom_line() + geom_smooth(se=FALSE) +
    facet_grid(age_bucket~SEX) +
    theme(legend.position = 'bottom')



# Separate plot removing White to see other races more clearly
asec_df_filtered |> 
    filter(educ_bucket == "Bachelor's or higher completed" & race_eth != 'White') |> 
    count(YEAR, age_bucket, SEX, race_eth, wt = ASECWT) |>
    mutate(`Relative Freq` = n/sum(n), .by = c('YEAR', 'age_bucket', 'SEX')) |> 
    ggplot(aes(x=YEAR, y=`Relative Freq`, color=race_eth)) + geom_line() + geom_smooth(se=FALSE) +
    facet_grid(age_bucket~SEX) +
...
asec_df_filtered |> 
    filter(educ_bucket == "Bachelor's or higher completed") |> 
    count(YEAR, age_bucket, SEX, race_eth, wt = ASECWT) |>
    mutate(freq = n/sum(n), .by = c('YEAR', 'age_bucket', 'SEX')) %>% 
    lm(data = ., freq ~  age_bucket + SEX + race_eth + YEAR + I(YEAR^2)) |> 
    summary()
```
