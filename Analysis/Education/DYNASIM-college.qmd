...
fehname0       = "Baseline"
fehdir0        = "C:/Users/dcosic/Dynasim/Dynasim-babybonds/Dynasim-core/run/output/base_v25/"
...
bb10k = fehdf |>
    filter(dataset=="Baby bonds") |>
    select(age, year, pernum, rsavings1) |>
    filter(age==18, rsavings1>=1e4) |>
    select(pernum) |>
    pull()
bb20k = fehdf |>
    filter(dataset=="Baby bonds") |>
    select(age, year, pernum, rsavings1) |>
    filter(age==18, rsavings1>=2e4) |>
    select(pernum) |>
    pull()
```
...
### Share with BA: Age Profile for Birth Cohort 2024

::: {.panel-tabset}

```{r}
#| label: bseduc-byrace-doby-tabsex
#| results: asis

plotdf = educByAgeBCohSexRace |>
    dplyr::filter(
            doby==2024,
            gradecat==16
        ) |>
...
plot_ycumshare_xage_crace_tsex <- function(sextab)
{
  knitr::knit_child(text = c(
    ...
    "plotdf |>",
    "  filter(sex==sextab) |>",
    "  ggplot2::ggplot(aes(x=age, y=cumshare,linetype=dataset,color=race)) + ",
...
res = purrr::map_chr(c("All", "Women", "Men"), plot_ycumshare_xage_crace_tsex)
cat(res, sep='\n')
...
### Share with BA: Age Profile for Birth Cohorts 2024-2033

::: {.panel-tabset}

```{r}
#| label: bseduc-byrace-dobycat-tabsex
#| results: asis

plotdf = educByAgeBCohcatSexRace |>
    dplyr::filter(
            dobycat=="2024-2033",
            gradecat==16
        ) |>
...
### Diff in Share with BA: Age Profile for Birth Cohorts 2024-2033

::: {.panel-tabset}

```{r}
#| label: diff-bseduc-byrace-tabsex
#| results: asis
...
plotdf = left_join(
    ...
    select(age, sex, race, cumshare0=cumshare),
    educByAgeBCohcatSexRace |>
    dplyr::filter(
            dobycat=="2024-2033",
            gradecat==16,
    ...
    select(age, sex, race, cumshare),
    by=c("age","sex","race")
) |>
    mutate(cumshare=cumshare-cumshare0)

plot_ydiffcumshare_xage_crace_tsex <- function(sextab)
{
  knitr::knit_child(text = c(
    "#### `r sextab`",
    "",
    ...
    "plotdf |>",
    "  filter(sex==sextab) |>",
    "  ggplot2::ggplot(aes(x=age, y=cumshare,color=race)) + ",
    "  geom_line() + ylab('Diff in share with BA (%)') +",
    "  scale_y_continuous(labels=function(x) {100*x})",
    "```",
    ""
    ), 
...
res = purrr::map_chr(c("All", "Women", "Men"), plot_ydiffcumshare_xage_crace_tsex)
cat(res, sep='\n')
...
### Share with BA: Age Profile for Birth Cohorts 2024-2033 and Those Who Had $10,000 or more in baby bonds at 18

::: {.panel-tabset}

```{r}
#| label: bseduc-byrace-dobycat-tabsex-bb10k
#| results: asis

plotdf = fehdf |>
    filter(age %in% ages, dobycat=="2024-2033", gradecat %in% grades, pernum %in% bb10k) |>
    sum_by_aser_time(dobycat, age) |>
    filter(gradecat==16) |>
    mutate(dataset=factor(dataset))
...
### Share with BA: Age Profile for Birth Cohorts 2024-2033 and Those Who Had $20,000 or more in baby bonds at 18

::: {.panel-tabset}

```{r}
#| label: bseduc-byrace-dobycat-tabsex-bb20k
#| results: asis

plotdf = fehdf |>
    filter(age %in% ages, dobycat=="2024-2033", gradecat %in% grades, pernum %in% bb20k) |>
    sum_by_aser_time(dobycat, age) |>
    filter(gradecat==16) |>
    mutate(dataset=factor(dataset))
