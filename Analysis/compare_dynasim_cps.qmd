...
plot_df = bind_rows(
  ...
  select(dyn_long, year, race_eth_cat, age_cat, sex_cat, educ_cat, weight) |> mutate(dataset='DYNASIM')) |>
    filter(educ_cat != "0-11th grades" & educ_cat != "More than Bachelor's")
...
plot_df |> 
    ...
    facet_grid(sex_cat~educ_cat, scales='free') +
...
plot_df |> 
    ...
    ggplot(aes(x=year, y=`Relative Freq`, color=dataset)) + geom_line() +
    facet_grid(race_eth_cat~educ_cat, scales='free') +
    theme(legend.position = 'bottom')
...
plot_df |> 
    ...
    ggplot(aes(x=year, y=`Relative Freq`, color=dataset)) + geom_line() +
    facet_grid(age_cat~educ_cat, scales='free') +
    theme(legend.position = 'bottom')
