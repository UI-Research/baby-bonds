...
basedf |>
  ...
  mutate(
      tmp=pnetworth,
      pnetworth = dyn_wealth_xform(tmp, type='linear'),
      pwealthpos  = dyn_wealth_xform(tmp, type='log+'),
      pwealthneg  = dyn_wealth_xform(tmp, type='log-')
  ) |>
  select(-tmp) |>
  filter(pnetworth>-20) |>
  group_by(race) |>
  summarize(across(c(pnetworth,pwealthpos,pwealthneg), ~ weighted.mean(.x, wt)))
)
```
...
```
### Enrollment After Dropping Out of College, Altenrative
...
### Dropping Out of College, Alternative
...
```
### Graduating from College, Alternative
