...
basedf |>
  ...
  pnw_make_3splines() |>
  pnw_make_logs() |>
  mutate(pnwawi = dyn_wealth_xform(pnetworth, type='linear')) |>
  filter(pnwawi>-20) |>
  group_by(race) |>
  summarize(
...
tab_marg_effects(estdf, "pnetworth")
```
...
```{r}
#| label: est-gsgrad-simple-models1
...
gsmodel_covars = c("yearf", "race", "asian", "pnetworth", "pared")

estdf = estdf |> estimate_model("tEnroll", gsmodel_covars, c(sex))

```

## Simple Model Estimated by Gender

```{r}
#| label: tbl-gsenrol-simple-mod-est1
...
## Simple Model Estimated by Gender and Race
...
gsmodel_covars = c("yearf", "asian", "pnetworth", "pared")
...
tab_marg_effects(estdf, c("pnetworth"))

```

### Simple Model Estimated by Gender and Race, Alternative
...
gsmodel_covars = c("yearf", "asian", "pnetworth", "pared", "has_child", "has_partner")
...
gsmodel_covars = c("yearf", "asian", "pnetworth", "pared", "has_child", "has_partner", "lnetworth*yearf")
