...
estdf = colenrdf |>
    ...
    mutate(
        ...
        pnetworth=dyn_wealth_xform(pnetworth, type='ihs')
    )
...
estdf = estdf |>
    ...
    mutate(
        mods = map(data, \(x) glm(
            tEnroll ~ yearf + asian + pnetworth + pnetworth_na, 
            data=x, 
            family=binomial)
        )
    ) |>
...
res_colenr = dyn_coef_to_df(models)
...
@tbl-enrol2-simple-mod-est shows the estimated coefficients.
...
estdf = estdf |>
    ...
    mutate(
        mods = map(data, \(x) glm(
            tEnroll ~ yearf + asian + pwealthpos + pwealthneg + pnetworth_na, 
            data=x, 
            family=binomial)
        )
    ) |>
...
estdf = estdf |>
    ...
    mutate(
        mods = map(data, \(x) glm(
            tDrop ~ yearf + asian + pnetworth + pnetworth_na, 
            data=x, 
            family=binomial)
        )
    ) |>
...
estdf = estdf |>
    ...
    mutate(mods = map(
        data, \(x) glm(
            tGrad ~ yearf + asian + pnetworth + pnetworth_na, 
            data=x, 
            family=binomial)
        )
    ) |>
