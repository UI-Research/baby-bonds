...
```{r}
#| label: fig-enrol-hazard-spells-sex-race
#| fig-cap: "Hazard function for college enrollment by enrollment spell, sex, and race. X-axes represents years since the graduation from high school, for the first spell of non-enrollment, and years, since dropping out of college for subsequent spells."
...
### Limiting the Number of Gap Years

Because the share of people who enroll in college 15 or more years after the high-school graduation is small (@fig-cum-share-gap-years), it might be good to limit the number of gap years (e.g., to 15).
...
dropdf |>
  group_by(id, spell) |>
  mutate(
    grad_spell = if_else(colgradyr %in% year, TRUE, FALSE),
    dropped = if_else(!grad_spell & spell_yr==max_spell_len, 1, dropped)
  ) |>
  select(id, year, enrolled, colenryr, colgradyr, spell, nspells, spell_yr, dropped, grad_spell) |>
  View()
