...
model_covars3 = c("yearf", "asian", "pnw1", "pnw2", "pnw3", "asian*pnw1", "asian*pnw2", "asian*pnw3", "pared", "asian*pared")
...
basedf = nlsy_get_base_df() |> 
    select(id, sex, race, hisp, hh_size, wt) |>
    filter(!is.na(race) & !is.na(hisp) & race!='No information') |>
    left_join(nlsy_get_imputed(), by="id") |>
    mutate(
        race = nlsy_recode_race_and_eth5(race, hisp),
...
```{r}
#| label: fig-pnetworthpc-density
#| fig-cap: "Distribution of parent's per-capita networth (divided by the average wage index) by race"

basedf |>
  select(id, pnetworth, pnetworthpc, race, wt) |>
  mutate(pnetworthpc = dyn_wealth_xform(pnetworthpc, type='linear')) |>
  filter(pnetworth>-20) |>
  ggplot() +
  geom_density(aes(x=pnetworthpc, y=after_stat(density), color=race, weight=wt), fill=NA) +
  xlab("PC net worth / AWI") + ylab("Density")
...
get_pnetworth_bins_basedf = function(data)
{
    return(basedf |>
      select(id, pnetworth, pnetworthpc, race, wt) |>
      mutate(
          pnetworth   = dyn_wealth_xform(pnetworth,   type='linear'),
          pnetworthpc = dyn_wealth_xform(pnetworthpc, type='linear')
          ) |>
      filter(pnetworth>-10) |>
      mutate(
          pnwbin   = cut(pnetworth,   seq(-19,22), labels=as.character(seq(-18,22))),
          pnwpcbin = cut(pnetworthpc, seq(-3,11,.5),  labels=as.character(seq(-2.5,11,.5)))
        )
    )
}
...
```{r}
#| label: fig-colgrads-by-pnetworthpc-bin
#| fig-cap: "The share of NLSY participants who graduated from 4-year college by binned parents' per-capita net worth"
...
plotdf |>
    ...
    ungroup() |>
    ggplot() +
    geom_col(aes(x=pnwpcbin, y=bs)) +
    geom_line(aes(x=pnwpcbin, y=dens, group=1), color='black') +
    xlab("Parents' per-capita networth (AWI)") +
    ylab("Share college grads")
```
...
```{r}
#| label: fig-ever-college-by-pnetworthpc-bin
#| fig-cap: "The share of NLSY participants who attended college by binned parents' per-capita net worth"
...
plotdf |>
    ...
    summarise(evercol = weighted.mean(evercol, wt), dens=max(dens)) |>
    ggplot() +
    geom_col(aes(x=pnwpcbin, y=evercol)) +
    geom_line(aes(x=pnwpcbin, y=dens, group=1), color='black') +
    xlab("Parents' per-capita networth (AWI)") +
    ylab("Share ever college")
```
...
```{r}
#| label: fig-ever-college-by-pnetworthpc-bin-race
#| fig-cap: "The share of NLSY participants who attended college by binned parents' per-capita net worth by race"

plotdf |>
    group_by(pnwpcbin, race) |>
    summarise(evercol = weighted.mean(evercol, wt)) |>
    ggplot() +
    geom_col(aes(x=pnwpcbin, y=evercol), position=position_dodge()) +
    xlab("Parents' per-capita networth (AWI)") +
    ylab("Share ever college") +
    facet_grid(rows=vars(race))
...
pnwpc_make_3splines = function(data)
{
  pnw_knots = c(0, 1.5)
  data = data |> mutate(
      x = dyn_wealth_xform(pnetworthpc, type='linear'),
      pnw1 = x,
      pnw2 = if_else(x>=pnw_knots[1], x-pnw_knots[1], 0),
      pnw3 = if_else(x>=pnw_knots[2], x-pnw_knots[2], 0)
  ) |>
...
basedf |>
  select(id, pnetworth, pnetworthpc, race, wt) |>
  pnwpc_make_3splines() |>
  rename_with(~gsub('^pnw', 'pnwpc', .x), starts_with("pnw")) |>
  pnw_make_3splines() |>
  pnw_make_logs() |>
  pnw_make_lin() |>
  filter(pnwawi>-20) |>
  group_by(race) |>
  summarize(
      across(c(pnetworth,pnwawi,pwealthpos,pwealthneg,pnw1,pnw2,pnw3,pnwpc1,pnwpc2,pnwpc3), 
             ~ weighted.mean(.x, wt))
      )
)
```
...
### College Enrollment with Piecewise-Linear Parents' Per-Capita Net-Worth

In this variation, we use piecewise linear splines for parents' per-capita networth. @tbl-enroll4-simple-mod-est shows the estimated coefficients.

```{r}
#| label: est-enroll4-models
...
```{r}
#| label: tbl-enroll4-simple-mod-est
#| tbl-cap: "Estimated coefficients for the model of college enrollment with parental networth split into negative an dpositive terms"
...
@tbl-enroll4-marginal shows average marginal effects of parental networth on the likelihood of enrollment. The numbers in the table can be interpreted as the effect that one percent increase in parental wealth has on the likelihood of enrollment in college.

```{r}
#| label: tbl-enroll4-marginal
#| tbl-cap: "Marginal effects of parental networth separate for negative and positive"
...
### Re-Enrollment with Piecewise-Linear Parents' Per-Capita Net-Worth

```{r}
#| label: est-reenrol4-models
...
```{r}
#| label: tbl-reenrol4-mod-est
#| tbl-cap: "Estimated coefficients of the re-enrollment model with piecewise-linear parental per-capita networth"
...
```{r}
#| label: tbl-reenroll4-marginal
#| tbl-cap: "Marginal effects of parental per-capita networth"
...
### Dropping out of College with Piecewise-Linear Parents' Per-Capita Net-Worth

```{r}
#| label: est-simple-drop4-models
...
```{r}
#| label: tbl-drop-simple4-mod-est
#| tbl-cap: "Estimated coefficients of the model of dropping out of college"
...
### College Graduation with Piecewise-Linear Parents' Per-Capita Net-Worth

```{r}
#| label: est-grad-simple4-models
...
```{r}
#| label: tbl-grad-simple4-mod-est
#| tbl-cap: "Estimated coefficients of the college graduation model"
...
### Graduate School by Gender and Race with Piecewiase Linear Per-Capita Parents' Wealth
