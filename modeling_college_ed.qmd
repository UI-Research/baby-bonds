...
basedf |>
  ...
  filter(pnetworth>-20) |>
  group_by(race) |>
  summarize(pnetworth = weighted.mean(pnetworth, wt))
)
```
...
#dyn_write_coef_file(gscoefs$df, gscoefs$filename, gscoefs$description)
...
estdf = gsenrdf |>
    ...
    mutate(
        yearf         = factor(pmin(timeEnroll,4)),
        pnetworth     = dyn_wealth_xform(pnetworth, type=opt_wealth_xform),
        income        = dyn_wealth_xform(income, type=opt_wealth_xform),
        lincome       = lag(income),
        networth      = dyn_wealth_xform(networth, type=opt_wealth_xform),
